...
void MapperIntIV::d_mmm(int32_t *res, const int32_t *mat_A, const int32_t *mat_B,
    ...
    for (size_t n = 0; n < n_matrix; n++) {
        ...
        for (size_t t_m = 0; t_m < tmp_size; ++t_m) {
            for (size_t k = 0; k < k_matrix; ++k) {
                tmp_out_int_[t_m] += (gd_p_[t_m][k] - gd_m_[t_m][k]) * mat_B[k * n_matrix + n];
            }
        }
        ...
        for (size_t m = 0; m < m_matrix; ++m) {
            for (size_t s = 0; s < split.size(); ++s) {
                res[m * n_matrix + n] += tmp_out_int_[m * split.size() + s] << shift_[s];
            }
        }
    }
}

void MapperIntIV::a_mmm(int32_t *res, const int32_t *mat_A, const int32_t *mat_B,
                        int32_t m_matrix, int32_t k_matrix, int32_t n_matrix) {
    // The splitted matrix is of size CFG.SPLITsize*M x N (CFG.SPLITsize values
    // per original matrix value) Two matrices exist: ia+ (ia_p_) and ia-
    // (ia_m_) The input is already positive only
    ...
    for (size_t n = 0; n < n_matrix; n++) {
        ...
        for (size_t i_bit = 0; i_bit < CFG.I_BIT; ++i_bit) {
            ...
            for (size_t t_m = 0; t_m < tmp_size; ++t_m) {
                for (size_t k = 0; k < k_matrix; ++k) {
                    tmp_out_fp_[t_m] +=
                        (ia_p_[t_m][k] - ia_m_[t_m][k]) * ((mat_B[k * n_matrix + n] >> i_bit) & 1);
                }
            }
